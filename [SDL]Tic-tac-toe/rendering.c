...
void render_grid(SDL_Renderer *renderer, const SDL_Color *color){
    SDL_SetRenderDrawColor(renderer, color->r, color->g, color->b, 255); //red, green, blue, alpha value to render
    for(int i = 1; i < N; i++){
        SDL_RenderDrawLine(renderer, i * CELL_WIDTH, 0,
                       i * CELL_WIDTH, SCREEN_HEIGHT);
        SDL_RenderDrawLine(renderer, 0, i * CELL_HEIGHT,
...
void render_x(SDL_Renderer *renderer, int row, int column, const SDL_Color *color){
    ...
    const float centre_x = CELL_WIDTH * 0.5 + column * CELL_WIDTH;
    const float centre_y = CELL_HEIGHT * 0.5 + row * CELL_HEIGHT;
    
    thickLineRGBA(renderer, centre_x - half_box_side, centre_y - half_box_side,
                  centre_x + half_box_side, centre_y + half_box_side,
                  8, color->r, color->g, color->b, 255);
    thickLineRGBA(renderer, centre_x + half_box_side, centre_y - half_box_side,
                  centre_x - half_box_side, centre_y + half_box_side,
                  8, color->r, color->g, color->b, 255);
}
...
void render_o(SDL_Renderer *renderer, int row, int column, const SDL_Color *color){
    ...
    const float centre_x = CELL_WIDTH * 0.5 + column * CELL_WIDTH;
    const float centre_y = CELL_HEIGHT * 0.5 + row * CELL_HEIGHT;
    filledCircleRGBA(renderer, centre_x, centre_y, half_box_side + 5, color->r, color->g, color->b, 255);
    filledCircleRGBA(renderer, centre_x, centre_y, half_box_side - 5, 0, 0, 0, 255);
}
